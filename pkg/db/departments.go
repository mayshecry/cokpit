...
func (s *Store) ListDepartments(ctx context.Context) ([]si.Department, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, name, description, created_at FROM departments ORDER BY name ASC, id ASC`)
	if err != nil {
	...
	for rows.Next() {
		var d si.Department
		var created int64
		if err := rows.Scan(&d.ID, &d.Name, &d.Description, &created); err != nil {
...
func (s *Store) DepartmentByID(ctx context.Context, id int64) (si.Department, error) {
	var d si.Department
	var created int64
	err := s.db.QueryRowContext(ctx, `SELECT id, name, description, created_at FROM departments WHERE id = ?`, id).Scan(&d.ID, &d.Name, &d.Description, &created)
...
func (s *Store) UserDepartments(ctx context.Context, userID int64) ([]si.Department, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT d.id, d.name, d.description, d.created_at FROM departments d JOIN user_departments ud ON ud.department_id = d.id WHERE ud.user_id = ? ORDER BY d.name ASC`, userID)
	if err != nil {
